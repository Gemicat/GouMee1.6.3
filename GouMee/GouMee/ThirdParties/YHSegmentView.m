...
- (UIView *)bottomLineView {
    if (!_bottomLineView) {
        _bottomLineView = [[UIView alloc] initWithFrame:CGRectMake(0, YH_SegmentViewHeight, _size.width, 1)];
//        _bottomLineView.backgroundColor = [UIColor groupTableViewBackgroundColor];
        _ScrollView_Y = YH_SegmentViewHeight + 1;
    }
...
- (UIScrollView *)segmentTitleView {
    if (!_segmentTitleView) {
        ...
        _segmentTitleView.backgroundColor = COLOR_STR(0xf5f5f5);
...
//        _indicateView.backgroundColor = YH_SegmentTintColor;
        ...
        [self.indicateView setGradientBackgroundWithColors:@[COLOR_STR(0xEA5F2C),COLOR_STR(0xffffff)] locations:nil startPoint:CGPointMake(0, 0) endPoint:CGPointMake(1, 0)];;
...
- (instancetype)initWithFrame:(CGRect)frame ViewControllersArr:(NSArray *)viewControllersArr TitleArr:(NSArray *)titleArr TitleNormalSize:(CGFloat)titleNormalSize TitleSelectedSize:(CGFloat)titleSelectedSize SegmentStyle:(YHSegementStyle)style ParentViewController:(UIViewController *)parentViewController ReturnIndexBlock:(yh_indexBlock)indexBlock {
    if (self = [super initWithFrame:frame]) {
        ...
        _titleArr = titleArr;
        _size = frame.size;
        Font_Default_size = titleNormalSize*kFitWidth;
        Font_Selected_size = titleSelectedSize*kFitWidth;
    ...
    if (style == YHSegementStyleIndicate) {
        ...
        for (int i = 0; i < _titleArr.count; i++) {
           ...
           CGSize titleSize = [title sizeWithAttributes:@{NSFontAttributeName: YH_Font_Selected}];
            ...
            [button setTitleColor:COLOR_STR(0x333333) forState:UIControlStateSelected];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    _indicateView.frame = CGRectMake(originMovedX + targetMovedWidth / _size.width * moved+(originButtonWidth + (targetButtonWidth - originButtonWidth) / _size.width * offset)/16, _indicateView.frame.origin.y,  (originButtonWidth + (targetButtonWidth - originButtonWidth) / _size.width * offset), _indicateView.frame.size.height);
}
...
- (void)setYh_indicateStyle:(YHSegementIndicateStyle)yh_indicateStyle {
    ...
    if (yh_indicateStyle == YHSegementIndicateStyleDefault) {
         ...
         _indicateView.frame = CGRectMake(_selectedButton.frame.origin.x, YH_SegmentViewHeight - YH_IndicateHeight-5, [self widthAtIndex:0], YH_IndicateHeight);
    }
}
