...
- (void)makeConstraints
{
    ...
    [_topControlsBar mas_makeConstraints:^(MASConstraintMaker *make) {
           make.left.right.equalTo(self);
           make.height.equalTo(@([UIApplication sharedApplication].statusBarFrame.size.height+40));
        make.top.mas_equalTo(0);
       }];
    [_backBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.bottom.mas_equalTo(_topControlsBar.mas_bottom).offset(0);
        make.left.mas_equalTo(_topControlsBar.mas_left).offset(0);;
...
- (UIView *)topControlsBar
{
    if (!_topControlsBar) {
        ...
        _topControlsBar.backgroundColor = COLOR_STR1(0, 0, 0, 1.0);
