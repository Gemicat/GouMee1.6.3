...
@implementation  HomeGoodsPuViewCell
...
-(void)addModel1:(NSDictionary *)model
{
  ...
  aa= [NSString stringWithFormat:@"%@",[Network removeSuffix:[NSString stringWithFormat:@"%.2f",(min*[div_Money floatValue]/100)]]];
    ...
    else
    {
        ...
        if (max >= 1000 || min >= 1000) {
            price =[NSString stringWithFormat:@"¥%@起", [Network removeSuffix:[priceArr valueForKeyPath:@"@min.floatValue"]]] ;
            NSMutableAttributedString *noteStr = [[NSMutableAttributedString alloc] initWithString:price];
            // 需要改变的区间(第一个参数，从第几位起，长度)
            NSRange range = NSMakeRange(0, 1);
            // 改变字体大小及类型
        ...
        {
            price =[NSString stringWithFormat:@"¥%@~%@", [Network removeSuffix:[priceArr valueForKeyPath:@"@min.floatValue"]], [Network removeSuffix:[priceArr valueForKeyPath:@"@max.floatValue"]]] ;
            NSMutableAttributedString *noteStr = [[NSMutableAttributedString alloc] initWithString:price];
            // 需要改变的区间(第一个参数，从第几位起，长度)
            NSRange range = NSMakeRange(0, 1);
            // 改变字体大小及类型
