...
- (void)viewDidLoad {
    ...
    [self.toolBar setTintColor:ThemeRedColor];
...
- (void)updatePreferredContentSizeWithTraitCollection:(UITraitCollection *)traitCollection {
    ...
    if (CGSizeEqualToSize(CGSizeZero, self.preferredContentSize)) {
        self.preferredContentSize = CGSizeMake(self.view.bounds.size.width, traitCollection.verticalSizeClass == UIUserInterfaceSizeClassCompact ? 220 : 300);
    }
}
...
- (void)setSelectedModelWithProvinceIndex:(NSInteger)provinceIndex
    ...
    if (citieIndex>self.model.selectedProvince.cityList.count) {
        citieIndex = [self.pickerView selectedRowInComponent:1];
    }
    self.model.selectedCitie = self.model.selectedProvince.cityList[citieIndex];
    if (areaIndex>self.model.selectedCitie.areaList.count) {
        areaIndex = [self.pickerView selectedRowInComponent:2];
    }
    self.model.selectedArea = self.model.selectedCitie.areaList[areaIndex];
}
...
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component{
    switch (component) {
        ...
        case 1: {
            return self.model.provinces[self.provinceIndex].cityList.count;
        }
        case 2: {
            return self.model.provinces[self.provinceIndex].cityList[self.citieIndex].areaList.count;
        };
        default: {
            return 0;
        }
...
- (NSString *)pickerView:(UIPickerView *)pickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component{
    switch (component) {
        ...
        case 1: {
            self.model.selectedCitie = self.model.provinces[self.provinceIndex].cityList[row];
            return self.model.selectedCitie.name;
        }
        case 2: {
            self.model.selectedArea = self.model.provinces[self.provinceIndex].cityList[self.citieIndex].areaList[row];
