...
- (CGFloat)titleWidth:(NSString *)title {
    ...
    if (_layoutStyle == MLMSegmentLayoutDefault) {
         return [title boundingRectWithSize:CGSizeMake(MAXFLOAT, CGRectGetHeight(self.frame)) options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName : [UIFont systemFontOfSize:sys_font]} context:nil].size.width + _singleW_Add+14;
    }
...
- (void)scrollViewSubviews:(UIScrollView*)scroll {
    ...
    if (titles && _headStyle != SegmentHeadStyleSlide) {
        ...
        if (_fontScale != 1) {
            curBtn.titleLabel.font = font1(@"PingFangSC-Semibold", scale(14));
        }
...
- (void)createBtn:(NSArray *)titlesArr addScroll:(UIScrollView*)scroll startX:(CGFloat)start_x start_index:(NSInteger)start_index {
    ...
    for (NSInteger i = start_index; i < titlesArr.count; i ++) {
        ...
        button.titleLabel.font = font1(@"PingFangSC-Regular", scale(14));
...
- (UIButton *)slideView {
    ...
    UIButton *slide = [[UIButton alloc] initWithFrame:CGRectMake(0, (SCROLL_HEIGHT-_slideHeight+16)/2, slide_w*_slideScale-h, _slideHeight-16)];

    UIButton *current_btn = buttonArray[currentIndex];
    slide.center = CGPointMake(current_btn.center.x, slide.center.y);
    slide.clipsToBounds = YES;
    slide.layer.cornerRadius = MIN(_slideCorner, _slideHeight/2-8);
    ...
    slideScroll.frame = CGRectMake(convertRect.origin.x, -(SCROLL_HEIGHT - _slideHeight+16)/2, SCROLL_WIDTH, SCROLL_HEIGHT);
...
- (void)changeSelectIndexPage:(NSInteger)index {
    {
        ...
        [UIView animateWithDuration:animation_time animations:^{
            ...
            if (_fontScale) {
                before_btn.titleLabel.font = font1(@"PingFangSC-Regular", scale(14));
                select_btn.titleLabel.font = font1(@"PingFangSC-Regular", scale(14));
            }
            ...
            if (slideView) {
                ...
                {
                     slideView.width = select_btn.width*_slideScale-20;
                }
...
- (void)fontChangeCurBtn:(UIButton *)curBtn nextBtn:(UIButton *)nextBtn changeScale:(CGFloat)changeScale{
    ...
    nextBtn.titleLabel.font = font1(@"PingFangSC-Regular", scale(14));
    curBtn.titleLabel.font = font1(@"PingFangSC-Regular", scale(14));
}
