...
-(void)creatCell
{
    ...
    [self.goodsIcon addSubview:self.chooseBtn];
    [self.chooseBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.left.mas_equalTo(scale(3));
    }];
    [self.chooseBtn setImage:[UIImage imageNamed:@"choose_select"] forState:UIControlStateSelected];
    [self.chooseBtn setImage:[UIImage imageNamed:@"choose_normal"] forState:UIControlStateNormal];
...
-(void)addModel:(NSDictionary *)model
{
 ...
 NSArray *arr = model[@"price_json"];
    ...
    NSString *aa = [NSString stringWithFormat:@"%@",[Network removeSuffix:@([model[@"good"][@"commission"] floatValue])]];
    NSString *sum_Money;
...
//    }
   ...
   if (status == 2) {
    ...
    NSString *time1 = [Network timestampSwitchTime:[Network timeSwitchTimestamp:model[@"live_time"] andFormatter:@"YYY-MM-dd HH:mm:ss"] andFormatter:@"MM.dd HH:mm"];
            self.startTime.backgroundColor = COLOR_STR1(215, 46, 81, 0.61);
              self.startTime.text =[NSString stringWithFormat:@"%@ 开播",time1];
