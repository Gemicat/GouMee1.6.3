...
#import "JyMessageViewCell.h"
#import "DateTranslate.h"
@implementation JyMessageViewCell
...
-(void)addModel:(NSDictionary *)model
{
    ...
    [self.goodsIcon sd_setImageWithURL:[NSURL URLWithString:model[@"good_pict_url"]]];
 ...
 self.context.text = [NSString stringWithFormat:@"不通过原因：%@",model[@"refuse_content"]];
            NSMutableAttributedString *noteStr = [[NSMutableAttributedString alloc] initWithString:self.context.text];

            [noteStr addAttribute:NSForegroundColorAttributeName value:COLOR_STR(0x999999) range:NSMakeRange(0,5)];
            [noteStr addAttribute:NSForegroundColorAttributeName value:ThemeRedColor range:NSMakeRange(6,_context.text.length-6)];

            self.context.attributedText = noteStr;
        ...
        else
        {
            ...
            [self.goodsView mas_updateConstraints:^(MASConstraintMaker *make) {
             ...
             h =h+ scale(44)+scale(15)+scale(12)+scale(60)+scale(12)+scale(0.5)+scale(12)+labelsize1.height;
        }
