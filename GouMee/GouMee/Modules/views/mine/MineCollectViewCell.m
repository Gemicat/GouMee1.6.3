...
@implementation MineCollectViewCell
...
-(void)creatCell
{
    ...
    self.goodsName.numberOfLines = 2;
     ...
     [_goodsName addSubview:self.brandLab];
     self.goodsName.lineBreakMode = NSLineBreakByWordWrapping;
        self.newsPrice = [UILabel new];
        [backView addSubview:self.newsPrice];
        self.oldsPrice = [UILabel new];
        [backView addSubview:self.oldsPrice];
        self.packageLab = [UILabel new];
        [backView addSubview:self.packageLab];

    self.vilidLab = [UILabel new];
    self.vilidLab.text = @"已失效";
    self.vilidLab.textColor = COLOR_STR(0x666666);
    self.vilidLab.font = font1(@"PingFangSC-Regular", scale(14));
    self.vilidLab.hidden = YES;
           [backView addSubview:self.vilidLab];
    ...
    [self.goodsName mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo (self.goodsIcon.mas_top).offset(scale(4));
  ...
  _lineView = [UIView new];
    ...
    self.newsPrice.textColor = ThemeRedColor;
    self.newsPrice.font = font(21);
    [self.newsPrice mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(self.goodsName.mas_left).offset(0);
        make.top.mas_equalTo(_lineView.mas_bottom).offset(scale(6));
    }];

    self.oldsPrice = [UILabel new];
    [backView addSubview:self.oldsPrice];
    self.oldsPrice.textColor = COLOR_STR(0xCCCCCC);
    self.oldsPrice.font = font(12);
    [self.oldsPrice mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(self.newsPrice.mas_right).offset(scale(5));
        make.bottom.mas_equalTo(self.newsPrice.mas_bottom).offset(scale(-4));
    }];
    ...
    [self.couponsView mas_makeConstraints:^(MASConstraintMaker *make) {
     make.top.mas_equalTo(self.newsPrice.mas_bottom).offset(scale(4));
        make.left.mas_equalTo(self.goodsName.mas_left).offset(0);
    ...
    [self.vilidLab mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.mas_equalTo(line.mas_bottom).offset(0);
            make.bottom.mas_equalTo(backView.mas_bottom).offset(0);
            make.left.mas_equalTo(line.mas_left).offset(0);
        }];
    self.moneyNum = [UIButton new];
    [backView addSubview:self.moneyNum];
    [self.moneyNum mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.mas_equalTo(self.vilidLab.mas_centerY).offset(0);
        make.height.mas_equalTo(scale(25));
        make.width.mas_equalTo(scale(80));
        make.right.mas_equalTo(line.mas_right).offset(0);
    }];
    self.moneyNum.layer.cornerRadius = scale(12.5);
    self.moneyNum.layer.masksToBounds = YES;

    [self.moneyNum setTitle:@"取消收藏" forState:UIControlStateNormal];
    self.moneyNum.titleLabel.font = font(14);
    [self.moneyNum setTitleColor:COLOR_STR(0x666666) forState:UIControlStateNormal];
    self.moneyNum.layer.borderColor = COLOR_STR(0x666666).CGColor;
    self.moneyNum.layer.borderWidth = 0.5;
...
{
    NSDictionary *goodModel = model[@"good"];
    
    if (isNotNull(goodModel[@"lock_rate"])) {
          NSString *rateStr = [Network removeSuffix:@([goodModel[@"lock_rate"] floatValue]/100)];
          NSString *start_time = [self getTimeFromTimestamp:[goodModel[@"lock_rate_start_time"] integerValue]];
          NSString *end_time = [self getTimeFromTimestamp:[goodModel[@"lock_rate_end_time"] integerValue]];
     ...
     if (isNotNull(goodModel[@"white_image"])) {
            [self.goodsIcon sd_setImageWithURL:[NSURL URLWithString:goodModel[@"white_image"]] placeholderImage:[UIImage imageNamed:@"goods_bg"]];
       }
       else
       {
            [self.goodsIcon sd_setImageWithURL:[NSURL URLWithString:goodModel[@"pict_url"]] placeholderImage:[UIImage imageNamed:@"goods_bg"]];
       }

           self.goodsName.text =[NSString stringWithFormat:@"%@",goodModel[@"title"]] ;
    self.goodsName.numberOfLines = 2;
    
    if ([goodModel[@"is_invalid"] intValue] == 1) {

        self.vilidLab.hidden = NO;
    ...
    if ([goodModel[@"volume"] integerValue] < 10000) {
           self.saleNum.text = [NSString stringWithFormat:@"月销量%@件",goodModel[@"volume"]];
       }
       else
       {
           self.saleNum.text = [NSString stringWithFormat:@"月销量%@万件",[Network notRounding:[goodModel[@"volume"] floatValue] afterPoint:2]];
       }
    ...
    if ([fuli[@"data_source"] integerValue] == 1) {
        NSString *afterPrice = [NSString stringWithFormat:@"¥%@",[Network removeSuffix:goodModel[@"final_price"] ]];
        ...
        NSNumber *a=[NSNumber numberWithFloat:[goodModel[@"final_price"] floatValue]];
        NSNumber *b=[NSNumber numberWithFloat:[goodModel[@"zk_final_price"] floatValue]];
        if ([a compare:b]==NSOrderedAscending) {
            NSMutableAttributedString *str = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"¥%@",[Network removeSuffix:goodModel[@"zk_final_price"]]]];
        ...
        if (isNotNull(goodModel[@"coupon_amount"])) {
            self.markLab.hidden = NO;
            self.couponsLab.text = [NSString stringWithFormat:@" %@ 元 ",[Network removeSuffix:goodModel[@"coupon_amount"] ]];
            [self.couponsView mas_updateConstraints:^(MASConstraintMaker *make) {
                make.left.mas_equalTo(self.markLab.mas_left).offset(0);
                make.right.mas_equalTo(self.couponsLab.mas_right).offset(0);
            }];
