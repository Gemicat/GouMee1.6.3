...
- (instancetype)init
{
    ...
    if (self) {
        ...
        _tipsLabel = [[UILabel alloc] initWithFrame:CGRectMake(5.0f, 0, self.frame.size.width - 10.0f, self.frame.size.height)];
...
#define ANIMATION_DURATION   0.1f      //动画时长
#define WINDOWVIEW_HEIGHT    300.0f    //显示视图高度
#define PICKERVIEW_HEIGHT    220.0f    //时间选择器高度
#define MARGIN               15.0f     //边距
#define LABEL_WIDTH          20.0f     //"至"label宽度
#define TEXTFIELD_WIDTH      ((SCREEN_WIDTH - MARGIN * 2.0f - LABEL_WIDTH) / 2.0f) //textfield宽度
#define TEXTFIELD_HEIGHT     50.0f     //textfield高度
#define BOTTOM_BUTTON_HEIGHT 44.0f     //底部按钮高度
#define BOTTOM_BUTTON_WIDTH  (SCREEN_WIDTH / 2.0f) //底部按钮宽度

#define FROM_TIME_BUTTON_PLACEHOLDER          @"请选择开始时间"
#define TO_TIME_BUTTON_PLACEHOLDER            @"设置直播排期时间"
...
//    [self showDatePickerAlertViewWithDateFormat:D_yyyyMMddHHmm datePickerMode:UIDatePickerModeDate selectCompletion:selectCompletion];
//}

+ (void)showDatePickerAlertViewWithDateFormat:(NSString *)dateFormat datePickerMode:(UIDatePickerMode)datePickerMode selectCompletion:(void (^)(NSDate *, NSDate *))selectCompletion withTime:(NSDate *)currentTime
{
    {
        ...
        [alertView resetActions:currentTime];
        [alertView show];
    }
}
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        [self.windowView setPosition:CGPointMake(0, SH) atAnchorPoint:CGPointZero];
        ...
        [self.datePicker setPosition:CGPointMake(0, CGRectGetMaxY(self.toTimeButton.frame)) atAnchorPoint:CGPointZero];
        [self.windowView addSubview:self.datePicker];

        [self.resetButton setPosition:CGPointMake(0, (WINDOWVIEW_HEIGHT-BOTTOM_SAFE_HEIGHT)) atAnchorPoint:CGPointMake(0, 1)];
        [self.windowView addSubview:self.resetButton];
        
        [self.ensureButton setPosition:CGPointMake(SCREEN_WIDTH, WINDOWVIEW_HEIGHT-BOTTOM_SAFE_HEIGHT) atAnchorPoint:CGPointMake(1, 1)];
        ...
        [line1 mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.mas_equalTo(self.ensureButton.mas_top).offset(0);
            make.left.mas_equalTo(self.windowView.mas_left).offset(0);
        ...
        tapGR.numberOfTouchesRequired = 1;
        [self addGestureRecognizer:tapGR];
        _dateFormat = D_yyyyMMddHHmm;
        
        [self resetAction];
    }
    return self;
}
- (void)resetActions:(NSDate *)data {
    [self.datePicker setDate:data animated:YES];
}
- (void)resetAction {
...
- (void)show {
    ...
    [UIView animateWithDuration:ANIMATION_DURATION animations:^{
        ...
        [self.windowView setPosition:CGPointMake(0, SCREEN_HEIGHT) atAnchorPoint:CGPointMake(0, 1)];
    } completion:^(BOOL finished) {
    }];
}
...
- (UIView *)windowView {
    if (!_windowView) {
        ...
        UIBezierPath *maskPath = [UIBezierPath bezierPathWithRoundedRect: _windowView.bounds byRoundingCorners:UIRectCornerTopLeft | UIRectCornerTopRight cornerRadii:CGSizeMake(10,10)];
        //创建 layer
        CAShapeLayer *maskLayer = [[CAShapeLayer alloc] init];
        maskLayer.frame = _windowView.bounds;
        //赋值
        maskLayer.path = maskPath.CGPath;
        _windowView.layer.mask = maskLayer;
    }
    return _windowView;
}
- (UIButton *)toTimeButton {
    if (!_toTimeButton) {
        _toTimeButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 10, TEXTFIELD_WIDTH, TEXTFIELD_HEIGHT)];
        [_toTimeButton setTitle:TO_TIME_BUTTON_PLACEHOLDER forState:UIControlStateNormal];
        [_toTimeButton setTitleColor:COLOR_STR(0x333333) forState:UIControlStateNormal];
        _toTimeButton.titleLabel.font = font1(@"PingFangSC-Medium", scale(14));
...
- (UIButton *)resetButton {
    if (!_resetButton) {
        ...
        [_resetButton setTitleColor:COLOR_STR(0x333333) forState:UIControlStateNormal];
        _resetButton.titleLabel.font = font1(@"PingFangSC-Medium", scale(14));
        _resetButton.backgroundColor = [UIColor whiteColor];
        
        [_resetButton addTarget:self action:@selector(hide) forControlEvents:UIControlEventTouchUpInside];
    }
    return _resetButton;
}
...
- (UIButton *)ensureButton {
    if (!_ensureButton) {
        ...
        [_ensureButton setTitleColor:ThemeRedColor forState:UIControlStateNormal];
        _ensureButton.titleLabel.font =font1(@"PingFangSC-Medium", scale(14));
