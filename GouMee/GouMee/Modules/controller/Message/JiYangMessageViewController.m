...
#import "NullView.h"
@interface JiYangMessageViewController ()<UITableViewDelegate,UITableViewDataSource>
{
    CGFloat cellHeight;
...
@implementation JiYangMessageViewController
-(NullView *)nullView
{
    if (!_nullView) {
...
-(void)getMessageListJson:(NSInteger)page
  {
      ...
      [parm setObject:@(2) forKey:@"notice_type"];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
     NSString *CellIdentifier =[NSString stringWithFormat:@"JyMessageViewCell%ld%ld",indexPath.section,indexPath.row] ;
    JyMessageViewCell *cell1 = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
    if (cell1 == nil) {
        cell1 = [[JyMessageViewCell alloc]
...
{
    JyMessageViewCell *cell = [self tableView:tableView cellForRowAtIndexPath:indexPath];
    return cell.height;
